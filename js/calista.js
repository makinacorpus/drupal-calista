(function ($, Drupal) {
   ...
   */
  Drupal.behaviors.calista = {
    attach: function (context) {
      // Prevent chrome bug with inputs inside anchors
      $('#calista-facets', context).find('a input').click(function () {
  ...
  Drupal.behaviors.calistaPane = {
    ...
    attach: function (context, settings) {
      $(context).find('#contextual-pane').once('calistaPane', function () {
        ...
        if ($.cookie('contextual-pane-hidden') && $.cookie('contextual-pane-hidden') !== 'false') {
          ...
          $toggle.find('a[href=#tab-' + settings.calista.defaultPane + ']').click();
        }

        $(window).resize(Drupal.behaviors.calistaPane.resizeTabs);
      });
    }
  };
